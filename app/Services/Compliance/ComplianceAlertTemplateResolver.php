...
final class ComplianceAlertTemplateResolver
{
    ...
    public function templateSettings(array $settings): array
    {
        ...
        foreach ($this->defaults() as $key => $defaults) {
            ...
            foreach (self::RICH_TEXT_FIELDS as $field) {
                $templates[$key][$field] = $this->sanitizer->sanitize($templates[$key][$field]);
            }

            $templates[$key]['subject'] = $this->sanitizer->plainText($templates[$key]['subject']);
        }
    ...
    public function recipientDefaultsFor(OverdueReport $report, array $settings = [], ?string $alertType = null): array
    {
        $family = $this->familyFor($report);
        $isOverdue = $alertType === ComplianceNotificationRun::ALERT_OVERDUE;
        $template = $family === self::FAMILY_ENGP
            ? ($isOverdue ? 'engp_overdue' : 'engp_due_soon')
            : ($isOverdue ? 'protected_area_overdue' : 'protected_area_due_soon');
        $content = $this->templateSettings($settings)[$template];

        return [
            'default_to' => $family === self::FAMILY_ENGP ? 'The concerned CENR Officer' : 'The OIC, PASu',
            'default_attention' => $isOverdue
                ? ($family === self::FAMILY_ENGP ? 'Chief, Conservation and Development Section and the ENGP Coordinator' : 'Chief, Conservation and Development Section')
                : ($family === self::FAMILY_ENGP ? 'Chief, Conservation and Development Section and the ENGP Coordinator' : ''),
    ...
    public function presentationFor(Collection $reports, ?string $alertType, array $settings = []): array
    {
        ...
        $recipientDefaults = $this->recipientDefaultsFor($reports->first(), $settings, $alertType);

        return [...$content, 'family' => $family, 'template' => $template, 'destination_label' => $isEngp ? 'Implementing Office' : 'Protected Area', 'group_heading_label' => $content['report_heading'] ?? ($isEngp ? 'Implementing Office' : 'Protected Area'), ...$recipientDefaults];
    }
}
