...
test('the permission seeder provisions the technical report contract for intended roles', function () {
    ...
    $staff = Role::findByName('Technical Staff');
    ...
    expect(collect($abilities)->every(fn (string $ability): bool => $admin->hasPermissionTo($ability)))->toBeTrue()
        ->and(collect(['technical-reports.view', 'technical-reports.create', 'technical-reports.update'])->every(fn (string $ability): bool => $staff->hasPermissionTo($ability)))->toBeTrue()
        ->and($staff->hasPermissionTo('technical-reports.delete'))->toBeFalse()
        ->and($viewer->hasPermissionTo('technical-reports.view'))->toBeTrue()
        ->and(collect(['technical-reports.create', 'technical-reports.update', 'technical-reports.delete'])->every(fn (string $ability): bool => ! $viewer->hasPermissionTo($ability)))->toBeTrue()
