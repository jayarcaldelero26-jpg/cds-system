...
function Summary({ workflows, rows = [] }) {
    const columns = [{ key: 'workflow_label', label: 'Workflow', render: row => <span className="font-bold dark:text-white">{row.workflow_label}</span> }, { key: 'office', label: 'Office' }, { key: 'record_source', label: 'Record Source' }, { key: 'period_label', label: 'Reporting Period' }, { key: 'deadline_submission', label: 'Deadline', render: row => date(row.deadline_submission) }, { key: 'date_released_cenro', label: 'Released by CENRO', render: row => date(row.date_released_cenro) }, { key: 'date_received_penro', label: 'Received by PENRO', render: row => date(row.date_received_penro) }, { key: 'timeliness_rating', label: 'Timeliness', render: row => badge(row.timeliness_rating) }, { key: 'monitoring_status', label: 'Monitoring Status', render: row => monitoringBadge(row.monitoring_status) }, { key: 'submission_status', label: 'Routing Status', render: row => badge(row.submission_status) }, { key: 'mov_status', label: 'MOV', render: row => show(row.mov_status) }];
...
function Tracker({ workflow, config, rows = {}, periods, periodsByYear, offices, years, filters, year, canCreate, canUpdate, canDelete }) {
    ...
    return <AuthenticatedLayout title={config.label}>
        ...
        <div className="mt-5 space-y-4">
            <div className="flex flex-col gap-3 rounded-2xl border border-gray-100 bg-white p-4 shadow-xl dark:border-gray-800 dark:bg-gray-900 sm:flex-row sm:items-end sm:justify-between"><div className="grid min-w-0 flex-1 gap-3 sm:grid-cols-3"><div className={label}><FloatingSelect id="engp-year-filter" label="Reporting Year" value={year} onChange={event => apply({ year: event.target.value, period_key: undefined })} size="sm"><option value="">Select year</option>{years.map(option => <option key={option} value={option}>{option}</option>)}</FloatingSelect></div><div className={label}><FloatingSelect id="engp-office-filter" label="Office" value={filters.office || ''} onChange={event => apply({ office: event.target.value || undefined })} size="sm"><option value="">All offices</option>{offices.map(office => <option key={office}>{office}</option>)}</FloatingSelect></div><div className={label}><FloatingSelect id="engp-period-filter" label="Reporting Period" value={filters.period_key || ''} onChange={event => apply({ period_key: event.target.value || undefined })} size="sm"><option value="">All periods</option>{periods.map(period => <option key={period.key} value={period.key}>{period.label}</option>)}</FloatingSelect></div></div>{canCreate && <button type="button" onClick={openCreate} className="shrink-0 rounded-xl bg-green-700 px-4 py-2.5 text-xs font-bold text-white shadow-sm hover:bg-green-800">+ Add Report</button>}</div>
...
function FormFields({ form, config, periods, periodsByYear, offices, selected, chooseMov, preview }) {
    const error = name => form.errors[name] && <span className="mt-1 block text-xs text-red-500">{form.errors[name]}</span>;
    const periodOptions = periodsByYear?.[String(form.data.reporting_year)] || periods;
    const change = name => event => {
        const value = event.target.value;
        form.setData(name, value);
        if (name === 'reporting_year') form.setData('period_key', periodsByYear?.[String(value)]?.[0]?.key || '');
    };
    ...
    return <>
        <CrudSection title="General / Report Information"><div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className={label}><FloatingSelect id="engp-period" label="Reporting Period" required value={form.data.period_key} onChange={change('period_key')}><option value="">Select Period</option>{periodOptions.map(period => <option key={period.key} value={period.key}>{period.label}</option>)}</FloatingSelect>{error('period_key')}</div>
        </div></CrudSection>
