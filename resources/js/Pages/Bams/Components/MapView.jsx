...
export default function MapView({
    ...
    console.log("Spatial Data Prop received in MapView:", spatialData);

    const safeRecords = Array.isArray(bamsRecords) ? bamsRecords : [];
    ...
    // --- SAFE PARSER & CONVERTER (Mosuporta sa Esri JSON ug Standard GeoJSON) ---
    const getValidGeoJson = () => {
        try {
            if (!spatialData) return null;
            let obj = typeof spatialData === 'string' ? JSON.parse(spatialData) : spatialData;

            if (!obj || typeof obj !== 'object') return null;

            // 1. Kon Esri JSON format ang nadawat (ArcGIS / BAMS format)
    ...
    return (
        <div className="max-w-7xl mx-auto bg-white dark:bg-gray-800 shadow-xl rounded-2xl overflow-hidden p-6 border border-gray-100 dark:border-gray-700 w-full transition-all space-y-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-gray-100 dark:border-gray-700 pb-4">
                <div>
                    <h3 className="text-xl font-extrabold text-gray-900 dark:text-white tracking-tight">BAMS Geographic Distribution Map</h3>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Visualizing Permanent Monitoring Plots (PMPs), spatial boundaries, and species observations.</p>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    >
                        <option value="All">All Records (Flora & Fauna)</option>
                        <option value="Flora">Flora / PMP Only 🌱</option>
            ...
            <div className="h-[720px] w-full rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-700 z-0 relative shadow-inner">
                            ...
                            <Popup>
                                <div className="p-1 space-y-1">
                                    <span className={`px-2 py-0.5 rounded-lg text-[10px] font-bold ${record.category === 'Fauna' ? 'bg-amber-100 text-amber-800' : 'bg-green-100 text-green-800'}`}>
                                        {record.category || 'Flora (PMP)'}
                                    </span>
                                    <p className="font-bold text-sm italic text-green-800">{record.species_code || record.species_name}</p>
                                    <p className="text-xs text-gray-600"><strong>Plot / Quadrat:</strong> {record.plot_no || 'N/A'} / {record.quadrat_no || 'N/A'}</p>
                                    <p className="text-xs text-gray-600"><strong>DBH:</strong> {record.dbh ? `${record.dbh} cm` : 'N/A'}</p>
                                    <p className="text-xs text-gray-500 font-mono pt-1">📍 {record.latitude}, {record.longitude}</p>
                                </div>
                            </Popup>
                        </Marker>
                    ))}
                ...
                {/* FLOATING MAP LEGEND */}
                {(validRecords.length > 0 || parsedSpatialData) && (
                    <div className="absolute bottom-6 left-6 z-[400] bg-white/95 dark:bg-gray-900/95 backdrop-blur-md p-3.5 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 text-xs space-y-2 no-print pointer-events-auto">
                        <p className="font-extrabold text-gray-800 dark:text-gray-200 border-b border-gray-100 dark:border-gray-800 pb-1">🗺️ BAMS Map Legend</p>
                        {parsedSpatialData && (
                            <div className="flex items-center gap-2.5 font-medium text-gray-700 dark:text-gray-300">
                                <span className="w-4 h-4 rounded bg-green-500/40 border border-green-700"></span>
                                <span>Imported Spatial Boundary (Shapefile/KML)</span>
                            </div>
                        )}
                        <div className="flex items-center gap-2.5 font-medium text-gray-700 dark:text-gray-300">
                            <span className="w-6 h-6 rounded-full bg-green-600 flex items-center justify-center text-white text-xs shadow-xs">🌱</span>
                            <span>Flora / Permanent Monitoring Plots</span>
                        </div>
                        <div className="flex items-center gap-2.5 font-medium text-gray-700 dark:text-gray-300">
                            <span className="w-6 h-6 rounded-full bg-amber-600 flex items-center justify-center text-white text-xs shadow-xs">🐾</span>
                            <span>Fauna Assessment Observations</span>
                        </div>
                    </div>
                )}
            </div>
