...
function EngpHeaderPeriod({ data = {} }) {
    ...
    return { title: (filters.year || new Date().getFullYear()) + ' Monitoring', subtitle: period?.label || frequency };
}

function EngpFilters({ data }) {
    const { filterOptions = {}, filters = {} } = data;
    const defaultYear = filterOptions.years?.[0] || new Date().getFullYear();
    const [values, setValues] = useState({ year: filters.year || defaultYear, period: filters.period || '', office: filters.office || '', frequency: filters.frequency || 'all' });
    useEffect(() => setValues({ year: filters.year || defaultYear, period: filters.period || '', office: filters.office || '', frequency: filters.frequency || 'all' }), [filters.year, filters.period, filters.office, filters.frequency, defaultYear]);
...
function PaContent({ rows = [], pagination = {}, filters = {}, paMatrix = [], topOverdueReports = [], executiveInterpretation = [] }) {
    return <><PaSubmissionTracking rows={rows} pagination={pagination} filters={filters} /><div className="grid gap-3 xl:grid-cols-2"><PaMatrix rows={paMatrix} /><TopOverdueCard overdue={topOverdueReports} /></div><PaInterpretation items={executiveInterpretation} /></>;
}

export default function Dashboard({ view = 'pa', engp, rows, pagination, filterOptions, filters, summary, paMatrix, topOverdueReports, executiveInterpretation }) {
    return <AuthenticatedLayout title="eDATS Monitoring Dashboard"><div className="space-y-3"><ViewTabs view={view} />{view === 'engp' ? <><EngpSummary data={engp || {}} /><EngpContent data={engp || {}} /></> : <><PaSummary filterOptions={filterOptions} filters={filters} summary={summary} /><PaContent rows={rows} pagination={pagination} filters={filters} paMatrix={paMatrix} topOverdueReports={topOverdueReports} executiveInterpretation={executiveInterpretation} /></>}</div></AuthenticatedLayout>;
}
