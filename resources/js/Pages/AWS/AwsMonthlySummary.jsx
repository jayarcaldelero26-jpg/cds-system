...
export default function AwsMonthlySummary({ rows = [], protectedAreas = [], filters = {}, yearOptions = [], monthOptions = [] }) {
    ...
    const headers = ['Reporting Period', 'Average Atmospheric Pressure (kPa)', 'Average Air Temperature (\u00B0C)', 'Average Vapor Pressure Deficit (kPa)', 'Average Relative Humidity (%)', 'Mean Wind Direction (\u00B0)', 'Total Precipitation (mm)', 'Average Wind Speed (m/s)', 'Data Completeness (%)', 'Remarks'];
    const rowValues = (row) => [row.period, number(row.average_atmospheric_pressure), number(row.average_air_temperature), number(row.average_vapor_pressure_deficit, 3), number(row.average_relative_humidity), number(row.mean_wind_direction), number(row.total_precipitation), row.data_completeness === null || row.data_completeness === undefined ? emptyMark : Number(row.data_completeness).toFixed(1) + '%', row.remarks];
    ...
    return <div className="space-y-4">
        <div className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
            ...
            <div className="grid gap-3 md:grid-cols-[repeat(3,minmax(0,1fr))_auto] md:items-end">
                ...
                <div className="flex flex-wrap gap-2"><button type="button" onClick={apply} className="rounded-xl bg-green-700 px-4 py-2.5 text-xs font-bold text-white hover:bg-green-800">Apply Filters</button><button type="button" onClick={reset} className="rounded-xl border border-gray-300 px-4 py-2.5 text-xs font-bold text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200">Reset</button></div>
            </div>
        ...
        {groupEntries.map((group) => <section key={String(group.name || 'unknown')} className="overflow-x-auto rounded-2xl border border-gray-100 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
            ...
            <table className="min-w-[1900px] w-full text-left text-xs"><thead className="bg-green-900 text-white"><tr>{headers.map((heading, index) => <th key={heading} className="px-3 py-3 font-bold">{index === 0 ? (form.mode === 'one_month' ? 'Reporting Date' : 'Reporting Period') : heading}</th>)}</tr></thead><tbody className="divide-y divide-gray-200 dark:divide-gray-800">{group.rows.map((row) => <tr key={String(row.protected_area_id) + '-' + row.period} className="align-top hover:bg-green-50/60 dark:hover:bg-green-950/30">{rowValues(row).map((value, index) => <td key={row.period + '-' + index} className={'px-3 py-3 ' + (index >= rowValues(row).length - 2 ? 'font-semibold' : '')}>{value}</td>)}</tr>)}</tbody></table>
        </section>)}
