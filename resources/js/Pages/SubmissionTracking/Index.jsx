...
export default function Index({ queues = {}, filters = {}, filterOptions = {}, trackingContext = {}, pagination = {} }) {
    ...
    const genericAction = selected?.routing?.actions?.find(item => item.key === form.data.stage);
    const columns = useMemo(() => [
        ...
        ] : trackingContext.is_cenro_user ? [{ key: 'turnaround', label: 'Turnaround', render: row => row.mov_processing?.turnaround?.label || FALLBACK }] : [{ key: 'submission_status', label: 'Current Routing Stage', render: row => <Badge value={row.routing?.current_status || row.submission_status} /> }, { key: 'current_document_location', label: 'Current Document Location', render: row => row.routing?.current_location || row.current_document_location || FALLBACK }, { key: 'timeliness', label: 'Timeliness', render: row => <Badge value={row.timeliness} /> }]),
    ...
    return <AuthenticatedLayout title="Submission Tracking"><PageHeader title="Submission Tracking" description="Monitor report / MOV routing milestones recorded by staff. These actions record real-world dates; eDATS does not transmit official documents." />
        <div className="mt-6 space-y-5">
            ...
            <div className="flex items-center justify-between rounded-2xl border border-gray-200 bg-white px-4 py-3 text-xs dark:border-gray-800 dark:bg-gray-900">
                ...
                <div className="flex gap-2">
                    {(pagination.current_page || 1) > 1 && <button type="button" className="rounded-lg border px-3 py-1.5 font-semibold" onClick={() => navigateFilters({ page: (pagination.current_page || 1) - 1 })}>Previous</button>}
                    {pagination.has_more && <button type="button" className="rounded-lg bg-green-700 px-3 py-1.5 font-semibold text-white" onClick={() => navigateFilters({ page: (pagination.current_page || 1) + 1 })}>Next</button>}
                </div>
            </div>
        </div>
        ...
        <CrudFormModal open={Boolean(correction)} mode="edit" title="Correct Routing Record" subtitle="Administrative correction. Every changed date is retained in the audit trail." onClose={() => !correctionForm.processing && setCorrection(null)} onSubmit={submitCorrection} processing={correctionForm.processing} errors={correctionForm.errors} saveLabel="Confirm Correction" maxWidth="max-w-xl">
            ...
            <CrudSection title="Current Routing Dates"><div className="space-y-3">{Object.prototype.hasOwnProperty.call(correctionForm.data.dates, 'date_report_released_cenro') && <DatePicker id="correction-cenro-release" label="CENRO Released" value={correctionForm.data.dates.date_report_released_cenro || ''} onChange={(value) => correctionForm.setData('dates', { ...correctionForm.data.dates, date_report_released_cenro: value })} error={correctionForm.errors['dates.date_report_released_cenro']} />}{Object.prototype.hasOwnProperty.call(correctionForm.data.dates, 'date_received_penro') && <DatePicker id="correction-penro-receipt" label="PENRO Received" value={correctionForm.data.dates.date_received_penro || ''} onChange={(value) => correctionForm.setData('dates', { ...correctionForm.data.dates, date_received_penro: value })} error={correctionForm.errors['dates.date_received_penro']} />}{Object.prototype.hasOwnProperty.call(correctionForm.data.dates, 'date_endorsed_regional') && <DatePicker id="correction-regional-endorsement" label="Regional Endorsed" value={correctionForm.data.dates.date_endorsed_regional || ''} onChange={(value) => correctionForm.setData('dates', { ...correctionForm.data.dates, date_endorsed_regional: value })} error={correctionForm.errors['dates.date_endorsed_regional']} />}{Object.entries(correctionForm.data.release_events || {}).map(([id, value]) => <DatePicker key={id} id={`correction-release-event-${id}`} label={`CENRO Release Event ${id}`} value={value || ''} onChange={(nextValue) => correctionForm.setData('release_events', { ...correctionForm.data.release_events, [id]: nextValue })} />)}</div></CrudSection>
