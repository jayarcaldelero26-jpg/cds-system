...
import SaveProgressIndicator from "@/Components/Attachments/SaveProgressIndicator";import { Link, useForm } from '@inertiajs/react';
...
export default function Form({ technicalReport, protectedAreas, reportTypes }) {
  ...
  const field = (name, label, type = 'text', required = false) => <FloatingInput id={`technical-${name}`} label={label} type={type} value={form.data[name] || ''} onChange={(event) => form.setData(name, event.target.value)} error={form.errors[name]} required={required} />;
  ...
  return <AuthenticatedLayout title={isEdit ? 'Edit General Report' : 'Add General Report'}>
        ...
        <div className="mt-6 grid items-start gap-6 xl:grid-cols-12">
            <Card className="xl:col-span-7"><form onSubmit={submit} className="space-y-7"><SaveProgressIndicator processing={form.processing} progress={form.progress} />
                <Section title="General Information & Report Details"><div className="grid gap-4 sm:grid-cols-2">
                    ...
                    {field('activity_name', 'Name of Activity', 'text', true)}
                    ...
                    <div className="sm:col-span-2"><DatePicker id="technical-date-conducted" label="Date Conducted" required value={canonicalDateConductedValue(form.data.date_conducted)} onChange={(value) => form.setData('date_conducted', value)} helperText={legacyDateConductedHelper(form.data.date_conducted)} error={form.errors.date_conducted} /></div>
                </div></Section>
                <Section title="Submission Information"><div className="grid gap-4 sm:grid-cols-2">{field('date_accomplished', 'Date Accomplished', 'date', true)}</div></Section>
                ...
                <Section title="MOV & Remarks"><div className="space-y-4"><div className="block text-sm font-medium text-gray-700 dark:text-gray-300"><label htmlFor="form-mov-supporting-document-pdf-doc-docx-xls-or-xlsx-maximum-20-mb" className="mb-1 block text-xs font-semibold text-gray-700 dark:text-gray-300">Report Attachment / MOV{(!isEdit || !existingFile) && <span className="ml-0.5 text-xs leading-4 text-red-500">*</span>}</label><p className="mb-1 text-xs font-semibold text-amber-700 dark:text-amber-300">{(!isEdit || !existingFile) && 'An MOV / supporting document is required.'}</p><AttachmentDropzone id="technical-report-attachment" label="Report Attachment / MOV" files={form.data.attachment ? [form.data.attachment] : []} existingAttachments={existingFile ? [existingFile] : []} multiple={false} required={!isEdit || !existingFile} accept=".pdf,.doc,.docx,.xls,.xlsx" acceptedTypesHint="PDF, DOC, DOCX, XLS, or XLSX" maxSizeHint="Maximum 20 MB" maxSizeBytes={20 * 1024 * 1024} onChange={selectFile} onSelectFile={setPreview} onRemoveExisting={() => {}} error={form.errors.attachment} disabled={form.processing} /></div>{preview && <div className="flex items-center justify-between rounded-xl border border-green-200 bg-green-50 px-3 py-2 text-sm text-green-800"><button type="button" onClick={() => setPreview(preview)} className="truncate font-semibold">{preview.name}</button>{form.data.attachment && <button type="button" onClick={removeFile} className="font-bold text-red-600">Cancel replacement</button>}</div>}{form.errors.attachment && <span className="text-xs text-red-600">{form.errors.attachment}</span>}<FloatingTextarea id="technical-remarks" label="Remarks" rows="4" value={form.data.remarks || ''} onChange={(event) => form.setData('remarks', event.target.value)} error={form.errors.remarks} /></div></Section>
