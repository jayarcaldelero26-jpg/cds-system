        ...
        <div className="mt-6 grid items-start gap-6 xl:grid-cols-12">
            <Card className="xl:col-span-7"><form onSubmit={submit} className="space-y-5"><SaveProgressIndicator processing={form.processing} progress={form.progress} />
                ...
                <CrudSection title="General / Report Information"><div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <div className={labelClass}><DatePicker id="form-date-conducted" label="Date Conducted" required value={canonicalDateConductedValue(form.data.date_conducted)} onChange={(value) => form.setData('date_conducted', value)} helperText={legacyDateConductedHelper(form.data.date_conducted)} error={form.errors.date_conducted} />{error('date_conducted')}</div>
                </div></CrudSection>
                <CrudSection title="Submission Information"><div className="grid grid-cols-1 gap-4 sm:grid-cols-2">{[['date_accomplished', 'Date Accomplished']].map(([name, text]) => <div key={name} className={labelClass}><DatePicker id={`management-plan-${name}`} label={text} required value={form.data[name]} onChange={(value) => form.setData(name, value)} />{error(name)}</div>)}</div>{calculated.length > 0 && <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">{calculated.map(([text, value]) => <div key={text} className="rounded-xl border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-900"><p className="mb-2 text-[11px] font-bold uppercase tracking-wide text-gray-500">{text}</p><Badge value={value} /></div>)}</div>}</CrudSection>
                <CrudSection title="Attachments & Remarks"><div className="space-y-5"><ManagementPlanAttachments manager={attachments} required={!isEdit || attachments.existingFiles.length === 0} error={form.errors.attachments || form.errors['attachments.0'] || form.errors.removed_attachments} previewClassName="xl:hidden" /><div className={labelClass}><FloatingTextarea id="form-remarks" label="Remarks" rows="4" value={form.data.remarks} onChange={change('remarks')} />{error('remarks')}</div></div></CrudSection>
