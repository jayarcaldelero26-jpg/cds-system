import { FileInput } from "@/Components/Crud/FileInput";import { FloatingSelect, FloatingInput, FloatingTextarea } from "@/Components/Form";import AuthenticatedLayout from '@/Layouts/AuthenticatedLayout';
...
import SharedTooltip from '@/Components/Tooltip';
...
export default function Index({ auth, bmsRecords, protectedAreas, filters, spatialLayers = [], annexHeaderMetadata, reportSubmissions, reportFilters, bmsThreats = [], initialTab }) {
  ...
  const submitGeoJsonImport = async (e) => {
    e.preventDefault();
    try {
      const normalized = await normalizeSpatialFile(geoJsonForm.data.file);
      geoJsonForm.transform(() => ({ protected_area_id: geoJsonForm.data.protected_area_id, layer_name: geoJsonForm.data.layer_name, spatial_geojson: JSON.stringify(normalized.geojson), source_format: normalized.source_format, original_filename: normalized.original_filename }));
      geoJsonForm.post(route('bms.import-geojson'), { preserveScroll: true, onSuccess: () => { geoJsonForm.reset(); setActiveTab('map'); } });
    } catch (error) { geoJsonForm.setError('file', error.message); }
  };
  ...
  return (
    <AuthenticatedLayout user={auth.user}>
          ...
          <MapView
            ...
            canManageSpatial={canManageBmsSpatial}
            onAddSpatialLayer={() => setActiveTab('geojson-import')}
          ...
          <CrudFormModal open mode="create" icon="🌿" title="Add Field Observation Data" subtitle="Create a BMS species observation from the Transect Data Summary." onClose={closeAddForm} onSubmit={submitRecord} processing={form.processing} errors={form.errors} saveLabel="Save Field Record" maxWidth="max-w-4xl">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 rounded-2xl border border-green-100 bg-green-50/50 p-4 dark:border-green-900 dark:bg-green-950/20">
                                    <div>
                                        ...
                                        <FloatingSelect id="index-protected-area" label="Protected Area" value={form.data.protected_area_id} onChange={(e) => form.setData('protected_area_id', e.target.value)} required>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <FloatingInput id="index-time-of-arrival" label="Time of Arrival" type="text" placeholder="e.g. 07:15 AM" value={form.data.time} onChange={(e) => form.setData('time', e.target.value)} />
                                    </div>
                                    <div>

                                        <FloatingSelect id="index-category" label="Category" value={form.data.category} onChange={(e) => form.setData('category', e.target.value)}>
                                            <option value="Flora">Flora</option>
                                            <option value="Fauna">Fauna</option>
                                        </FloatingSelect>
                                    </div>
                                    <div>
                                        ...
                                        <FloatingInput id="index-taxonomic-group" label="Taxonomic Group" type="text" placeholder="e.g. Birds, Trees" value={form.data.taxonomic_group} onChange={(e) => form.setData('taxonomic_group', e.target.value)} required />
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                    <div className="md:col-span-2">
                                        ...
                                        <FloatingInput id="index-scientific-name" label="Scientific Name" type="text" placeholder="e.g. Agathis philippinensis" value={form.data.species_scientific_name} onChange={(e) => form.setData('species_scientific_name', e.target.value)} required />
                                        {fieldError(form.errors, 'species_scientific_name')}
                                    </div>
                                    <div className="md:col-span-2">

                                        <FloatingInput id="index-local-common-name" label="Local / Common Name" type="text" placeholder="e.g. Almaciga" value={form.data.species_common_name} onChange={(e) => form.setData('species_common_name', e.target.value)} />
                                    </div>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>

                                        <FloatingInput id="index-count-no-of-individuals" label="Count / No. of Individuals" type="text" placeholder="e.g. 1, 2, Dominant, Flock" value={form.data.count} onChange={(e) => form.setData('count', e.target.value)} required />
                                        {fieldError(form.errors, 'count')}
                                    </div>
                                    <div>

                                        <FloatingSelect id="index-mode-of-observation" label="Mode of Observation" value={form.data.mode_of_observation} onChange={(e) => form.setData('mode_of_observation', e.target.value)}>
                                ...
                                <div className="bg-gray-50 dark:bg-gray-900/40 p-4 rounded-2xl border border-gray-200 dark:border-gray-700 space-y-4">
                                    ...
                                    {coordType === 'DD' &&
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div><FloatingInput id="index-latitude" label="Latitude" type="text" value={form.data.latitude} onChange={(e) => form.setData('latitude', e.target.value)} /></div>
                                            <div><FloatingInput id="index-longitude" label="Longitude" type="text" value={form.data.longitude} onChange={(e) => form.setData('longitude', e.target.value)} /></div>
                                        </div>
              }
                                    {coordType === 'DMS' &&
              <div className="space-y-3 text-xs">
                                            <div>
                                                ...
                                                <div className="grid grid-cols-4 gap-2 mt-1">
                                                    <FloatingInput id="index-deg" label="Deg" type="number" placeholder="Deg" value={latDeg} onChange={(e) => setLatDeg(e.target.value)} />
                                                    <FloatingInput id="index-min" label="Min" type="number" placeholder="Min" value={latMin} onChange={(e) => setLatMin(e.target.value)} />
                                                    <FloatingInput id="index-sec" label="Sec" type="number" placeholder="Sec" value={latSec} onChange={(e) => setLatSec(e.target.value)} />
                                                    <FloatingSelect id="latitude-direction" label="Latitude Direction" value={latDir} onChange={(e) => setLatDir(e.target.value)}><option value="N">N</option><option value="S">S</option></FloatingSelect>
                                                </div>
                                            </div>
                                            <div>
                                                <span className="font-bold text-gray-700 dark:text-gray-300">Longitude (DMS):</span>
                                                <div className="grid grid-cols-4 gap-2 mt-1">
                                                    <FloatingInput id="index-deg" label="Deg" type="number" placeholder="Deg" value={lonDeg} onChange={(e) => setLonDeg(e.target.value)} />
                                                    <FloatingInput id="index-min" label="Min" type="number" placeholder="Min" value={lonMin} onChange={(e) => setLonMin(e.target.value)} />
                                                    <FloatingInput id="index-sec" label="Sec" type="number" placeholder="Sec" value={lonSec} onChange={(e) => setLonSec(e.target.value)} />
                                                    <FloatingSelect id="longitude-direction" label="Longitude Direction" value={lonDir} onChange={(e) => setLonDir(e.target.value)}><option value="E">E</option><option value="W">W</option></FloatingSelect>
                                                </div>
                                            </div>
                                        </div>
              }
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
                                            <div><FloatingInput id="index-utm-zone" label="UTM Zone" type="text" placeholder="e.g. 51N" value={utmZone} onChange={(e) => setUtmZone(e.target.value)} /></div>
                                            <div><FloatingInput id="index-easting-x" label="Easting (X)" type="text" placeholder="e.g. 562300" value={easting} onChange={(e) => setEasting(e.target.value)} /></div>
                                            <div><FloatingInput id="index-northing-y" label="Northing (Y)" type="text" placeholder="e.g. 769200" value={northing} onChange={(e) => setNorthing(e.target.value)} /></div>
                                        </div>
              }
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div><FloatingInput id="index-elevation-masl" label="Elevation (masl)" type="text" placeholder="e.g. 453 - 683" value={form.data.elevation} onChange={(e) => form.setData('elevation', e.target.value)} /></div>
                                    <div><FloatingInput id="index-remarks" label="Remarks" type="text" placeholder="Additional notes" value={form.data.remarks} onChange={(e) => form.setData('remarks', e.target.value)} /></div>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div><FloatingInput id="index-location" label="Location" type="text" value={form.data.location} onChange={(e) => form.setData('location', e.target.value)} /></div>
                                    <div><FloatingInput id="index-observer" label="Observer" type="text" value={form.data.observer_name} onChange={(e) => form.setData('observer_name', e.target.value)} /></div>
                                </div>
                        </CrudFormModal>
          }
          ...
          <div className="max-w-2xl mx-auto bg-white dark:bg-gray-800 shadow-xl rounded-2xl p-8 border border-gray-100 dark:border-gray-700">
                            ...
                            <form onSubmit={submitImport} className="space-y-5">
                                <div>
                                    ...
                                    <FloatingSelect id="index-protected-area" label="Protected Area" value={importForm.data.protected_area_id} onChange={(e) => importForm.setData('protected_area_id', e.target.value)} required>
                                        <option value="">Select Protected Area</option>
                                        {protectedAreas.map((pa) => <option key={pa.id} value={pa.id}>{pa.name}</option>)}
                                    </FloatingSelect>
          ...
          <div className="max-w-2xl mx-auto bg-white dark:bg-gray-800 shadow-xl rounded-2xl p-8 border border-gray-100 dark:border-gray-700">
                            ...
                            <form onSubmit={submitGeoJsonImport} className="space-y-5">
                                <div>
                                    ...
                                    <FloatingSelect id="index-protected-area" label="Protected Area" value={geoJsonForm.data.protected_area_id} onChange={(e) => geoJsonForm.setData('protected_area_id', e.target.value)} required>
                                        <option value="">Select Protected Area</option>
                                        {protectedAreas.map((pa) => <option key={pa.id} value={pa.id}>{pa.name}</option>)}
                                    </FloatingSelect>
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 max-w-2xl w-full shadow-2xl border border-gray-200 dark:border-gray-700 animate-pop-in max-h-[90vh] overflow-y-auto">
                        ...
                        <form onSubmit={submitHeaderEdit} className="space-y-4 text-sm">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div><FloatingInput id="index-end-gps-reading" label="End GPS Reading" type="text" value={annexHeaderForm.data.end_gps} onChange={(e) => annexHeaderForm.setData('end_gps', e.target.value)} /></div>
                                <div><FloatingInput id="index-weather-condition" label="Weather Condition" type="text" value={annexHeaderForm.data.weather_condition} onChange={(e) => annexHeaderForm.setData('weather_condition', e.target.value)} /></div>
                                <div><FloatingInput id="index-elevation-masl" label="Elevation (MASL)" type="text" value={annexHeaderForm.data.elevation} onChange={(e) => annexHeaderForm.setData('elevation', e.target.value)} /></div>
                                <div><FloatingInput id="index-ecosystem-type" label="Ecosystem Type" type="text" value={annexHeaderForm.data.ecosystem_type} onChange={(e) => annexHeaderForm.setData('ecosystem_type', e.target.value)} /></div>
                                <div>

                                    <FloatingInput id="index-species-observed" label="Species Observed"
                type="text"
      ...
      <CrudFormModal open mode="edit" icon="✏️" title="Edit Species Record" subtitle={editingRecord.species_scientific_name} onClose={() => {editForm.clearErrors();setActionError('');setIsEditingRecord(false);}} onSubmit={submitEdit} processing={editForm.processing} errors={editForm.errors} canDelete={canDeleteBms} onDelete={() => setShowDeleteConfirm(true)} maxWidth="max-w-4xl">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                ...
                                <div><FloatingInput id="index-time-of-arrival" label="Time of Arrival" type="text" value={editForm.data.time} onChange={(e) => editForm.setData('time', e.target.value)} /></div>
                                <div>

                                    <FloatingSelect id="index-category" label="Category" value={editForm.data.category} onChange={(e) => editForm.setData('category', e.target.value)}><option value="Flora">Flora</option><option value="Fauna">Fauna</option></FloatingSelect>
                                </div>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                <div><FloatingInput id="index-scientific-name" label="Scientific Name" type="text" value={editForm.data.species_scientific_name} onChange={(e) => editForm.setData('species_scientific_name', e.target.value)} required />{fieldError(editForm.errors, 'species_scientific_name')}</div>
                                <div><FloatingInput id="index-common-name" label="Common Name" type="text" value={editForm.data.species_common_name} onChange={(e) => editForm.setData('species_common_name', e.target.value)} /></div>
                                <div><FloatingInput id="index-taxonomic-group" label="Taxonomic Group" type="text" value={editForm.data.taxonomic_group} onChange={(e) => editForm.setData('taxonomic_group', e.target.value)} required /></div>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <div><FloatingInput id="index-count-abundance" label="Count / Abundance" type="text" value={editForm.data.count} onChange={(e) => editForm.setData('count', e.target.value)} required />{fieldError(editForm.errors, 'count')}</div>
                                <div>

                                    <FloatingSelect id="index-mode-of-observation" label="Mode of Observation" value={editForm.data.mode_of_observation} onChange={(e) => editForm.setData('mode_of_observation', e.target.value)}><option value="Seen">Seen</option><option value="Heard">Heard</option><option value="Seen/Heard">Seen/Heard</option></FloatingSelect>
                                </div>
                            </div>
                            <div className="bg-gray-50 dark:bg-gray-900/40 p-4 rounded-2xl border border-gray-200 dark:border-gray-700 space-y-4">
                                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                                ...
                                {editCoordType === 'DD' &&
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div><FloatingInput id="index-latitude" label="Latitude" type="text" value={editForm.data.latitude} onChange={(e) => editForm.setData('latitude', e.target.value)} /></div>
                                        <div><FloatingInput id="index-longitude" label="Longitude" type="text" value={editForm.data.longitude} onChange={(e) => editForm.setData('longitude', e.target.value)} /></div>
                                    </div>
          }
                                {editCoordType === 'DMS' &&
          <div className="space-y-3 text-xs">
                                        <div>
                                            <span className="font-bold text-gray-700 dark:text-gray-300">Latitude (DMS):</span>
                                            <div className="grid grid-cols-4 gap-2 mt-1">
                                                <FloatingInput id="index-deg" label="Deg" type="number" placeholder="Deg" value={editLatDeg} onChange={(e) => setEditLatDeg(e.target.value)} />
                                                <FloatingInput id="index-min" label="Min" type="number" placeholder="Min" value={editLatMin} onChange={(e) => setEditLatMin(e.target.value)} />
                                                <FloatingInput id="index-sec" label="Sec" type="number" placeholder="Sec" value={editLatSec} onChange={(e) => setEditLatSec(e.target.value)} />
                                                <FloatingSelect id="edit-latitude-direction" label="Latitude Direction" value={editLatDir} onChange={(e) => setEditLatDir(e.target.value)}><option value="N">N</option><option value="S">S</option></FloatingSelect>
                                            </div>
                                        </div>
                                        <div>
                                            <span className="font-bold text-gray-700 dark:text-gray-300">Longitude (DMS):</span>
                                            <div className="grid grid-cols-4 gap-2 mt-1">
                                                <FloatingInput id="index-deg" label="Deg" type="number" placeholder="Deg" value={editLonDeg} onChange={(e) => setEditLonDeg(e.target.value)} />
                                                <FloatingInput id="index-min" label="Min" type="number" placeholder="Min" value={editLonMin} onChange={(e) => setEditLonMin(e.target.value)} />
                                                <FloatingInput id="index-sec" label="Sec" type="number" placeholder="Sec" value={editLonSec} onChange={(e) => setEditLonSec(e.target.value)} />
                                                <FloatingSelect id="edit-longitude-direction" label="Longitude Direction" value={editLonDir} onChange={(e) => setEditLonDir(e.target.value)}><option value="E">E</option><option value="W">W</option></FloatingSelect>
                                            </div>
                                        </div>
                                    </div>
          }
                                {editCoordType === 'UTM' &&
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
                                        <div><FloatingInput id="index-utm-zone" label="UTM Zone" type="text" placeholder="e.g. 51N" value={editUtmZone} onChange={(e) => setEditUtmZone(e.target.value)} /></div>
                                        <div><FloatingInput id="index-easting-x" label="Easting (X)" type="text" placeholder="e.g. 562300" value={editEasting} onChange={(e) => setEditEasting(e.target.value)} /></div>
                                        <div><FloatingInput id="index-northing-y" label="Northing (Y)" type="text" placeholder="e.g. 769200" value={editNorthing} onChange={(e) => setEditNorthing(e.target.value)} /></div>
                                    </div>
          }
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <div><FloatingInput id="index-elevation-masl" label="Elevation (masl)" type="text" value={editForm.data.elevation} onChange={(e) => editForm.setData('elevation', e.target.value)} /></div>
                                <div><FloatingInput id="index-observer" label="Observer" type="text" value={editForm.data.observer_name} onChange={(e) => editForm.setData('observer_name', e.target.value)} /></div>
                                <div><FloatingInput id="index-location" label="Location" type="text" value={editForm.data.location} onChange={(e) => editForm.setData('location', e.target.value)} /></div>
                            </div>
                            <div><FloatingTextarea id="index-remarks" label="Remarks" rows="3" value={editForm.data.remarks} onChange={(e) => editForm.setData('remarks', e.target.value)} />{fieldError(editForm.errors, 'remarks')}</div>
                </CrudFormModal>
      }
