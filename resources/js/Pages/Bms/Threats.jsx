...
export default function Threats({ threats = [], protectedAreas = [] }) {
  ...
  return <div className="space-y-4">
        ...
        <CrudFormModal open={modal === 'create' || modal === 'edit'} mode={modal === 'edit' ? 'edit' : 'create'} icon="⚠️" title={modal === 'edit' ? 'Edit BMS Threat' : 'Add BMS Threat'} subtitle="Record the persisted threat classification, location, coordinates, and response." onClose={backFromForm} onSubmit={submit} processing={form.processing} errors={form.errors} canDelete={modal === 'edit' && canDelete} onDelete={() => setDeleteTarget(selectedThreat)} maxWidth="max-w-4xl">
            <CrudSection title="Threat Information"><div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><div className={label}><FloatingSelect id="threats-protected-area" label="Protected Area" value={form.data.protected_area_id} onChange={(event) => form.setData('protected_area_id', event.target.value)} required><option value="">Select Protected Area</option>{protectedAreas.map((area) => <option key={area.id} value={area.id}>{area.name}</option>)}</FloatingSelect>{error('protected_area_id')}</div>{field('date', 'Date', { type: 'date', required: true })}{field('location', 'Location')}{field('threat_type', 'Threat Type', { required: true })}{field('threat_detail', 'Threat Detail')}{field('extent', 'Extent / Scale of Impact')}{field('severity', 'Severity')}</div></CrudSection>
            <CrudSection title="Coordinates"><div className="space-y-4"><div className={label}><FloatingSelect id="threats-coordinate-format" label="Coordinate Format" value={form.data.coord_format} onChange={(event) => form.setData('coord_format', event.target.value)}><option value="DD">Decimal Degrees (DD)</option><option value="DMS">Degrees, Minutes, Seconds (DMS)</option><option value="UTM">UTM Zone</option></FloatingSelect>{error('coord_format')}</div>{form.data.coord_format === 'DD' && <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">{field('latitude', 'Latitude')}{field('longitude', 'Longitude')}</div>}{form.data.coord_format === 'DMS' && <div className="grid grid-cols-3 gap-3">{field('lat_deg', 'Latitude Degrees')}{field('lat_min', 'Latitude Minutes')}{field('lat_sec', 'Latitude Seconds')}{field('long_deg', 'Longitude Degrees')}{field('long_min', 'Longitude Minutes')}{field('long_sec', 'Longitude Seconds')}</div>}{form.data.coord_format === 'UTM' && <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">{field('utm_zone', 'UTM Zone')}{field('easting', 'Easting')}{field('northing', 'Northing')}</div>}</div></CrudSection>
            <CrudSection title="Response & Remarks"><div className="space-y-4">{field('actions_taken', 'Actions Taken')}<div className={label}><FloatingTextarea id="threats-remarks" label="Remarks" rows="4" value={form.data.remarks} onChange={(event) => form.setData('remarks', event.target.value)} />{error('remarks')}</div></div></CrudSection>
        </CrudFormModal>
