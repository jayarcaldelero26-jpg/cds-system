...
export default function PambMovProgress({ row, context = {}, onSubmit, onReview, onRelease }) {
    ...
    return <section className="space-y-3 rounded-xl border border-green-100 bg-green-50/60 p-3 dark:border-green-900/60 dark:bg-green-950/20" aria-label="MOV processing progress">
        ...
        {(progress.review_history || []).length > 0 && <div id="pamb-cenro-review-history" className="border-t border-green-100 pt-2 dark:border-green-900/60"><p className="text-[10px] font-bold uppercase tracking-wide text-gray-500">CENRO MOV Review History</p>{progress.review_history.map((item, index) => <p key={`${item.event_key}-${index}`} className="mt-1 text-[11px] text-gray-600 dark:text-gray-300">{item.event_label} · {item.recorded_by || 'Recorded user'}{item.recorded_role || item.recorded_office ? ` · ${[item.recorded_role, item.recorded_office].filter(Boolean).join(' · ')}` : ''} · {item.recorded_at ? formatReportDateTime(item.recorded_at, FALLBACK) : FALLBACK}{item.remarks ? ` · ${item.remarks}` : ''}</p>)}</div>}
    </section>;
}
