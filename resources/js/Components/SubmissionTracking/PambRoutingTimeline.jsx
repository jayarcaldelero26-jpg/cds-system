...
function Marker({ status }) {
    ...
    if (status === "current")
        return (
            <span
                className="edats-current-stage-marker relative z-0 mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center"
                aria-hidden="true"
            >
    ...
    if (status === "not_applicable")
        return (
            <span
                className="mt-1 flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-gray-100 text-[10px] text-gray-500 dark:bg-gray-700 dark:text-gray-300"
                aria-hidden="true"
            >
                —
            </span>
    ...
    return (
        <div className="min-w-0 rounded-lg border border-gray-100 bg-gray-50/80 px-3 py-2 dark:border-gray-700 dark:bg-gray-900/50">
            ...
            <p className="mt-1 break-words text-sm font-medium text-gray-900 dark:text-white">
                {date ? eventDate(value) : value || FALLBACK}
            </p>
        </div>
    );
}

export default function PambRoutingTimeline({
    row,
    onRecord,
    onCanonicalAction,
    actions = null,
        ...
        <div className="space-y-4">
            <section className="space-y-3">
                <div>
                    ...
                    <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                        A concise view of the document's current owner, delay,
                ...
                <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <SummaryItem
                        ...
                        value={summary.responsible_office}
                    />
                    <SummaryItem
                        label="Pending Since"
                        value={summary.pending_since}
                        date
                    />
                    <SummaryItem
                        label="Working Days Pending"
                        value={
                            summary.working_days_pending === null ||
                            summary.working_days_pending === undefined
                                ? null
                                : `${summary.working_days_pending} working day${summary.working_days_pending === 1 ? "" : "s"}`
                        }
                    />
                    <SummaryItem
                        label="Next Expected Action"
                        value={summary.next_expected_action}
                    />
                    <SummaryItem
                        label="Last Updated"
                    ...
                    <SummaryItem
                        label="Recorded By"
                        value={lastAction.recorded_by}
                    />
                </div>
                {lastAction.label && (
                    <div className="rounded-lg border border-green-100 bg-green-50/60 px-3 py-2 text-xs dark:border-green-900/60 dark:bg-green-950/20">
                        <p className="font-bold uppercase tracking-wide text-green-800 dark:text-green-300">
                            Last Action
                        </p>
                        <p className="mt-1 font-semibold text-gray-900 dark:text-white">
                            {lastAction.label} ·{" "}
                            {eventDate(lastAction.occurred_at)}
                        </p>
                ...
                {review?.applicable && (
                    <section
                        className="rounded-lg border border-blue-100 bg-blue-50/60 px-3 py-2 text-xs dark:border-blue-900/60 dark:bg-blue-950/20"
                        aria-label="CENRO review verdict"
                    >
                        <div className="flex flex-wrap items-center justify-between gap-2">
                            <p className="font-bold uppercase tracking-wide text-blue-800 dark:text-blue-300">
                                CENRO Review Verdict
                            </p>
                            <span className="rounded-full bg-blue-100 px-2 py-1 font-bold text-blue-800 dark:bg-blue-900/60 dark:text-blue-100">
                                {review.verdict}
                            </span>
                        </div>
                        <div className="mt-2 grid gap-2 sm:grid-cols-2">
                            ...
                            <SummaryItem
                                ...
                                value={review.originating_office}
                            />
                            <SummaryItem
                                label="Reviewed On"
                        ...
                        {review.correction_reason && (
                            <p className="mt-2 whitespace-pre-wrap text-amber-800 dark:text-amber-200">
                                <span className="font-semibold">
                                    Correction Reason:
                                </span>{" "}
                                {review.correction_reason}
                        ...
                        {review.previous_correction_cycles > 0 && (
                            <p className="mt-2 text-gray-600 dark:text-gray-300">
                                <span className="font-semibold">
                                    Previous Correction Cycles:
                                </span>{" "}
                                {review.previous_correction_cycles}
                            </p>
                        )}
                        {review.previous_correction && (
                            <p className="mt-2 text-gray-600 dark:text-gray-300">
                                <span className="font-semibold">
                                    Previous Correction:
                                </span>{" "}
                                Needs Correction —{" "}
                                {review.previous_correction.reason ||
            ...
            <section className="space-y-3">
                ...
                {row.cenro_release_applicable === false && (
                    <p className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-200">
                        CENRO Release: N/A — PENRO-managed protected area. The
                    ...
                    <ol className="divide-y divide-gray-100 dark:divide-gray-700">
                        {(row.routing_timeline || []).map((stage) => {
                            ...
                            const verdictActions =
                                current &&
                                String(stage.key || "").startsWith(
                                    "received_by_penro_final",
                                )
                                    ? [
                                          ...(verdictFlags.can_return_for_penro_correction
                                              ? [
                                                    {
                                                        key:
                                              ...
                                              : []),
                                          ...(verdictFlags.can_approve_for_regional_release
                                              ? [
                                                    {
                                                        key:
                                                            "penro_final_approved_for_regional" +
                                                            verdictCycle,
                                                        label: "Approve for Regional Release",
                                                    },
                                                ]
                                              : []),
                                      ]
                            ...
                            return (
                                <li
                                    key={stage.key}
                                    className="flex items-start gap-3 py-2 first:pt-1 last:pb-1"
                                >
                                    <Marker status={stage.status} />
                                    <div className="min-w-0 flex-1">
                                        <div className="flex flex-wrap items-start justify-between gap-x-3 gap-y-1">
                                            <p
                                                className={`text-sm ${stage.status === "completed" || current ? "font-semibold text-gray-900 dark:text-white" : "font-medium text-gray-500 dark:text-gray-400"}`}
                                            >
                                                {stage.administrative_override && (
                                        ...
                                        {delay && (
                                            <p className="mt-0.5 text-[11px] font-medium text-green-700 dark:text-green-300">
                                                {delay}
                                            </p>
                                        )}
                                        {pending && (
                                        ...
                                        />
                                        {stage.remarks && (
                                            <p className="mt-1 whitespace-pre-wrap text-[11px] italic text-gray-500 dark:text-gray-400">
                                                {stage.remarks}
                                            </p>
                                        )}
                                    </div>
            ...
            <section className="space-y-3">
                <h3 className="text-xs font-bold uppercase tracking-wider text-green-800 dark:text-green-400">
                    Informational Turnaround Metrics
                </h3>
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                    {[
                        ["CENRO → PENRO", "cenro_to_penro"],
                        ["PENRO → Regional", "penro_to_regional"],
                        ["CENRO → Regional", "cenro_to_regional"],
                        [
                            "Total Pending at PENRO",
                            "total_working_days_pending_at_penro",
                        ],
                    ].map(([label, key]) => (
                        <div
                            key={key}
                            className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 dark:border-gray-700 dark:bg-gray-800/70"
                        >
                            <p className="text-[10px] font-bold uppercase leading-4 text-gray-500 dark:text-gray-400">
                                {label}
                            </p>
                ...
                <p className="text-[11px] text-gray-500 dark:text-gray-400">
                    Turnaround metrics use the PAMB business calendar and do not
                    affect compliance status, deadline, timeliness, or alert
                    eligibility.
                </p>
            </section>
        </div>
