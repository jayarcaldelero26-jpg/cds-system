...
function isInline(attachment) {
    return ['application/pdf', 'image/jpeg', 'image/png'].includes(String(attachment?.mime_type || attachment?.type || '').toLowerCase());
}
...
export default function DocumentPreviewDialog({ open, row, onClose }) {
    ...
    const attachment = row?.current_document || row?.effective_document || row?.mov_attachment;
    const url = attachment?.preview_url || attachment?.url || row?.mov_url;
