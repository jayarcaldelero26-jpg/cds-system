...
export default function RoutingAttachmentField({ currentDocument, file, onChange, error, disabled, processing, uploadProgress, attachmentAllowed = true, correctionAttachment = false }) {
    ...
    return <div className="space-y-4">
        ...
        {attachmentAllowed && <><section className="space-y-2"><p className="text-[10px] font-extrabold uppercase tracking-wide text-gray-500">{correctionAttachment ? 'Correction Attachment (Optional)' : 'Updated Copy Dropzone'}</p><AttachmentDropzone id="routing-attachment" label={correctionAttachment ? 'Correction / Reference Attachment (Optional)' : 'Updated / Signed / Stamped Copy (Optional)'} files={hasSelectedAttachment ? [file] : []} onChange={onChange} accept=".pdf,.jpg,.jpeg,.png,.doc,.docx" acceptedTypesHint="PDF, JPG, JPEG, PNG, DOC, or DOCX" maxSizeBytes={100 * 1024 * 1024} maxSizeHint="Maximum 100 MB" disabled={disabled} error={error} helperText={correctionAttachment ? 'Attach a marked-up, received, endorsed, or reference copy when needed.' : 'Attach a newly signed, stamped, or updated document copy only when applicable.'} /></section><TransferProgress state={activeUploadProgress} />
