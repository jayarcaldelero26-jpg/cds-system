...
<div style="font-size:14px;line-height:1.15;margin:0 0 16px;text-align:justify;">{!! $richText->render($presentation['introductory_text'] ?? '', $memoParagraphStyle) !!}</div>
<div style="font-size:14px;line-height:1.15;margin:0 0 20px;text-align:justify;">{!! $richText->render($presentation['instruction_text'] ?? '', str_replace('16px;', '20px;', $memoParagraphStyle)) !!}</div>
...
<div style="font-size:14px;line-height:1.15;margin:16px 0 8px;"><strong>{{ $destinationLabel }}:</strong> {{ $isEngp ? $group['target_office'] : $group['protected_area_name'] }}</div>
...
@endforeach
<div style="font-size:14px;line-height:1.15;margin:24px 0 18px;text-align:justify;">{!! $richText->render($presentation['closing_text'] ?? '', str_replace('16px;', '18px;', $memoParagraphStyle)) !!}</div>
<div style="font-size:14px;line-height:1.15;margin:0 0 30px;font-weight:700;text-align:justify;">{!! $richText->render($presentation['closing_directive'] ?? 'PLEASE BE GUIDED ACCORDINGLY.', str_replace('16px;', '30px;', $memoParagraphStyle)) !!}</div>
@else
...
<div style="font-size:14px;line-height:1.15;margin:0 0 18px;text-align:justify;">{!! $richText->render($openingText, str_replace('16px;', '18px;', $memoParagraphStyle)) !!}</div>
<div style="font-size:14px;line-height:1.15;margin:0 0 18px;font-weight:700;text-align:justify;">{!! $richText->render($presentation['instruction_text'] ?? 'Below is/are the pending report(s) that require immediate submission:', str_replace('16px;', '18px;', $memoParagraphStyle)) !!}</div>
@foreach ($groups as $group)
<div style="margin:22px 0 8px;{{ $groupHeadingStyle }}font-size:15px;line-height:1.15;font-weight:700;">{!! $richText->render($isEngp ? $groupHeadingLabel : $group['protected_area_name']) !!}</div>
...
@endforeach
<div style="font-size:14px;line-height:1.15;margin:24px 0 10px;text-align:justify;">{!! $richText->render($presentation['closing_text'] ?? $settings['compliance_warning_text'], str_replace('16px;', '10px;', $memoParagraphStyle)) !!}</div><div style="font-size:14px;line-height:1.15;margin:0 0 30px;font-weight:700;text-align:justify;">{!! $richText->render($presentation['closing_directive'] ?? 'FOR INFORMATION AND STRICT COMPLIANCE, PLEASE.', str_replace('16px;', '30px;', $memoParagraphStyle)) !!}</div>
@endif
