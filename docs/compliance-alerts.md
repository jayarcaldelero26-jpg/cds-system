...
Preview current mapped data in CDS at **Compliance Alerts**, or run a safe audit scan:

```powershell
php artisan compliance:send-overdue-alerts --dry-run
php artisan schedule:list
```

`--dry-run` records an auditable dry-run result and never sends mail.
...
Test mail is separately gated by `COMPLIANCE_TEST_EMAIL_ENABLED=true` and a configured test recipient. It sends only to that test address, is marked `[TEST]`, and never affects automatic daily deduplication.

When no current overdue reports exist, **Preview Test** and **Send Test Email** use a clearly labelled in-memory controlled fixture. It does not create or alter any tracker record.

Useful environment settings include:

```env
COMPLIANCE_ALERTS_ENABLED=false
COMPLIANCE_TEST_EMAIL_ENABLED=false
COMPLIANCE_ALERTS_SEND_TIME=08:00
COMPLIANCE_ALERTS_TIMEZONE=Asia/Manila
COMPLIANCE_ALERTS_TEST_RECIPIENT_EMAIL=
```
...
## Controlled live-email test

Do not activate PA, CENRO, or automatic production delivery for this procedure.

1. Keep `COMPLIANCE_ALERTS_ENABLED=false`.
2. Temporarily set `COMPLIANCE_TEST_EMAIL_ENABLED=true`.
3. In Compliance Alerts settings, configure `test_recipient_email` with an authorized internal/test mailbox. It must not be a production PA/CENRO address.
4. Clear cached configuration if applicable:

   ```powershell
   php artisan optimize:clear
   ```

5. Verify the safe scan:

   ```powershell
   php artisan compliance:send-overdue-alerts --dry-run
   ```

6. In the Compliance Alerts UI, review the **TEST EMAIL** safety panel, then use **Send Test Email** and confirm the displayed destination.
7. Inspect the received message and history log:
   - `[TEST]` subject and test mailbox in TO
   - no production recipient or CC address
   - memorandum formatting, report accuracy, deadlines, days overdue, signatory, and footer
   - test run status in Notification History
8. After the controlled test, set `COMPLIANCE_TEST_EMAIL_ENABLED=false`.
9. Clear configuration again:

   ```powershell
   php artisan optimize:clear
   ```

Do not enable `COMPLIANCE_ALERTS_ENABLED` as part of this checklist.
